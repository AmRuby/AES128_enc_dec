...
entity dual_mem is  
  ...
  port (clk  : in std_logic;  
        ...
        a    : in std_logic_vector(ADDR_LENGTH - 1 downto 0);    -- address
        dpra : in std_logic_vector(ADDR_LENGTH - 1 downto 0);    -- another optional address as dual memory takes two addrices 
        di   : in std_logic_vector(DATA_LENGTH - 1 downto 0);    -- data in
...
architecture rtl of dual_mem is 
  type ram_type is array (0 to N_ADDR-1) of std_logic_vector (DATA_LENGTH-1 downto 0);  
  signal RAM : ram_type ;
  signal read_a : std_logic_vector(ADDR_LENGTH - 1 downto 0);  
  signal read_dpra : std_logic_vector(ADDR_LENGTH - 1 downto 0);
  
  signal S_BOX_EN : std_logic := '1';   -- it allows the dual memory to save the S_BOX values.  
...
begin  
  ...
  spo <= RAM(conv_integer(read_a));         --two values at the two input addresses. 
